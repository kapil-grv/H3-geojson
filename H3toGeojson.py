...
#df = h3_to_geo_boundary('8abe0dd9285ffff')
#print(df)
...
if __name__ == "__main__":
    ...
    with open(output_file, 'w') as f:
        f.write(str('{ "type": "FeatureCollection", "features": [ \n'))

    with open(output_file, 'a+') as fpout:
        inp = csv.DictReader(open(input_file))
        count=0
        header = 'H3_11' #Header of column containing H3 name
        prop1 = 'ID' #Property number 1
        prop2 = '' #Property number 2
        for i in inp:
            df = h3_to_geo_boundary(i[header])
            poly = Polygon([df])
            count = count+1
            features = Feature(properties={"prop1": i[prop1],"H3_index": i[header]}, geometry=poly)
            fpout.write(str(features)+',\n')
